...
import { faAngleDown, faAngleUp, faChartArea, faChartBar, faChartLine, faFlagUsa, faFolderOpen, faGlobeEurope, faPaperclip, faUserPlus,faStethoscope,faBedPulse } from '@fortawesome/free-solid-svg-icons';
...
// import ProfileCover from "../assets/profilePic/profile-cover.jpg";
...
export const CounterWidget = (props) => {
  const { icon, iconColor, category, title, period, percentage,area } = props;
      ...
      <Card.Body>
        <Row className="d-block d-xl-flex align-items-center">
          ...
          <Col xs={12} xl={7} className="px-xl-0">
            ...
            <small>{period}, <FontAwesomeIcon icon={faGlobeEurope} size="xs" /> {area}</small>
...
export const TeamMembersWidget = (props) => {

  const TeamMember = (props) => {
    const { name, statusKey, image, icon, btnText } = props;
    const status = {
      online: { color: "success", label: "Available" },
      inMeeting: { color: "warning", label: "In a Appointment" },
      offline: { color: "danger", label: "Not Available" }
    };
    ...
    <Card border="light" className="shadow-sm">
      ...
      <Card.Body>
        <ListGroup className="list-group-flush list my--3">
          {DoctorList.map(tm => <TeamMember key={`team-member-${tm.id}`} {...tm} />)}
        </ListGroup>
      </Card.Body>
    </Card>
  );
  ...
  return (
    <Card border="light" className="shadow-sm">
      ...
      <Card.Body>
        ...
        <Progress title="Doctor" color="purple" icon={faStethoscope} percentage={34} />
        <Progress title="Patient" color="danger" icon={faBedPulse} percentage={60} />
        <Progress title="Nurse" color="tertiary" icon={faVuejs} percentage={45} />
        <Progress title="Hygienist" color="info" icon={faReact} percentage={35} />
